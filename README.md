...
- Original GameBot Home card with bundled start artwork and paginated Help Center UX
...
1. Copy `.env.example` to `.env` and fill in your real values. **Never commit `.env` or bot tokens.**
...
Python 3.11+ is recommended. The bot runtime uses **Aiogram 3.x** for Telegram transport/handlers, with SQLite for persistence and pytest for validation.
...
- Optional Telegram custom emoji IDs can be configured with `EMOJI_*`; normal Unicode remains the safe fallback. For message/caption text you can also write Telegram custom emoji directly, e.g. `No Way, Really? <tg-emoji emoji-id="5422712788944434896">😂</tg-emoji>`. Inline buttons use Aiogram's `icon_custom_emoji_id` field when an emoji ID is configured.
